...
gff_file <- "/data/users/ltucker/A_thaliana_Taz-0_organization_and_annotation/results/01a_EDTA_annotation/assembly.fasta.mod.EDTA.TEanno.gff3"
...
if (!is.null(gypsy_data) && nrow(gypsy_data) > 0) {
    ...
    circos.genomicDensity(gypsy_data, col = c("#00800080"), track.height = 0.1, window.size = 1e6, count_by = "number")
}

if (!is.null(copia_data) && nrow(copia_data) > 0) {
    message("Plotting Copia data...")
    circos.genomicDensity(copia_data, col = c("#8B000080"), track.height = 0.1, window.size = 1e6, count_by = "number")
}

if (!is.null(hat_data) && nrow(hat_data) > 0) {
    circos.genomicDensity(hat_data, col = c("#FF000080"), track.height = 0.1, window.size = 1e6, count_by = "number")
}

if (!is.null(cacta_data) && nrow(cacta_data) > 0) {
    circos.genomicDensity(cacta_data, col = c("#0000FF80"), track.height = 0.1, window.size = 1e6, count_by = "number")
}

if (!is.null(mutator_data) && nrow(mutator_data) > 0) {
    circos.genomicDensity(mutator_data, col = c("#FFA50080"), track.height = 0.1, window.size = 1e6, count_by = "number")
}

if (!is.null(helitron_data) && nrow(helitron_data) > 0) {
    circos.genomicDensity(helitron_data, col = c("#80008080"), track.height = 0.1, window.size = 1e6, count_by = "number")
}

circos.clear()

lgd <- Legend(title = "TE Superfamily", at = c("Gypsy", "Copia", "hAT", "CACTA", "Mutator", "Helitron"),
    legend_gp = gpar(fill = c("#00800080", "#8B000080", "#FF000080", "#0000FF80", "#FFA50080", "#80008080"))
)
